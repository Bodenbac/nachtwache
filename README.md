...
One Block, Nachtwellen, der Sammler. Konzept siehe „Inselkette Konzept" im Vault (v3.2). Dieses Paket enthält das fertige Datapack (`out/nachtwache.zip`), das Ressourcenpaket für die Spieler (`out/nachtwache-rp.zip`), den Generator (`build.py`, ruft `rp_build.py` mit auf) und die Tabellen, aus denen das Datapack gebaut wird (`tabellen/`).
